...
template <class T>
SparseTable<T>::SparseTable(const vector<T>& data,const function <T(T,T)>& combine){
    ...
    for (int p = 1; (1 << p)<=n; ++p)
        for (int i = 0; i+(1<<p) <= n; ++i)
            table[i][p] = this->combine(table[i][p-1],table[i+(1<<(p-1))][p-1]);
}
...
T SparseTable<T>::query(int l,int r){
    ...
    return this->combine(table[l][lg],table[r-(1<<lg)+1][lg]);
}
